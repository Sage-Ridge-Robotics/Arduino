...
void ScanForBlocks()
{
  if (millis() - lastMove > 20){
    ...
    if ((panLoop.m_pos >= RCS_MAX_POS)||(panLoop.m_pos <= RCS_MIN_POS)) {
      ...
      if (scanIncrement < 0) {
        myDCMotorL->setSpeed(200); myDCMotorR->setSpeed(200); myDCMotorL->run(BACKWARD); myDCMotorR->run(FORWARD);
      }
      else {
        myDCMotorL->setSpeed(175); myDCMotorR->setSpeed(175); myDCMotorL->run(FORWARD); myDCMotorR->run(BACKWARD);
      }
