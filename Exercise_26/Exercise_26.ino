...
A GPS-enabled battle bot with compass. This sketch provides an array (table) of lat and long
...
// Instantiate! ------------------------------------------------------------------------------------------
Adafruit_GPS GPS(&mySerial);                                           // GPS subsystem
  ...
  if (millis() - timer > 5000) { 
    ...
    if (GPS.fix) {    
      ...
      float compassBearing = (atan2(event.magnetic.y,event.magnetic.x) * 180) / PI;
       ...
       //Normalize
       if (compassBearing < 0) {compassBearing = 360 + compassBearing;}      
